...
import { buildChaosPrompt } from "./chaos.js";
import {
  addGuildMemberRole,
...
import { getLocalDayKey, getLocalMonthKey, jsonResponse } from "./utils.js";
...
function formatAttemptResult(result) {
  if (result === "passed") return "zaliczone";
  if (result === "retry") return "druga szansa";
  if (result === "timeout") return "timeout";
  if (result === "failed") return "niezaliczone";
  return result || "brak";
}

function buildVerificationHistoryMessage(config, attempts, userId = "") {
  if (!attempts.length) {
    return userId
      ? `Brak zapisanej historii dla <@${userId}>.`
      : "Brak zapisanej historii weryfikacji.";
  }

  const lines = [
    userId ? `**Historia weryfikacji dla <@${userId}>**` : "**Ostatnie weryfikacje na serwerze**",
    "",
  ];

  for (const attempt of attempts) {
    const modeLabel = String(
      attempt.details?.challenge?.modeLabel || attempt.details?.challenge?.mode || attempt.mode || "nieznany tryb",
    ).trim();
    const summary = String(attempt.details?.evaluation?.summary || "").trim();
    lines.push(
      [
        `Uzytkownik: <@${attempt.userId}>`,
        `Wynik: **${formatAttemptResult(attempt.result)}**`,
        `Tryb: **${modeLabel}**`,
        `Srednia: **${attempt.averageScore}/100**`,
        `Temat: **${attempt.interest}**`,
        `Kiedy: ${formatTimestamp(attempt.createdAt, config.resetTimezone)}`,
        summary ? `Podsumowanie: ${summary}` : null,
      ]
        .filter(Boolean)
        .join("\n"),
    );
...
function buildEventStatusMessage(config, monthKey, dayKey, scheduledCount, upcomingEvents) {
  ...
  if (scheduledCount < 1) {
    lines.push("Plan eventow jest pusty. Uruchom `/generate_monthly_events` albo poczekaj na cron.");
    return lines.join("\n");
  }

  lines.push(
    `Przy obecnym ustawieniu \`MONTHLY_EVENT_COUNT=${config.monthlyEventCount}\` event nie musi pojawiac sie codziennie.`,
    "",
  );

  if (!upcomingEvents.length) {
    lines.push("Brak kolejnych eventow w bazie.");
    return lines.join("\n");
  }

  lines.push("**Najblizsze eventy:**", "");
  for (const event of upcomingEvents) {
    const postedLabel = event.postedAt ? "wyslany" : "oczekuje";
    lines.push(
      `- ${event.eventDate} | **${event.title}** | typ: \`${event.kind}\` | status: ${postedLabel}`,
    );
  }
...
async function handleApplicationCommand(interaction, env, ctx, config) {
  ...
  if (commandName === "chaos_prompt") {
    const category = String(getCommandOptionValue(interaction, "category") || "random");
    return createPublicMessageResponse(buildChaosPrompt(category));
  }
      ...
      (async () => {
        try {
          ...
          await editOriginalInteractionResponse(config, interactionToken, {
            content:
              result.posted > 0
                ? `Opublikowano ${result.posted} event(y) na dzis w kanale <#${config.eventChannelId}>.`
                : `Na dzis nie bylo nic zaplanowanego albo wszystko juz zostalo wyslane. Kanal eventow: <#${config.eventChannelId}>.`,
            components: [],
          });
        } catch (error) {
          await editOriginalInteractionResponse(config, interactionToken, {
  ...
  if (commandName === "verification_history") {
    const requestedUserId = String(getCommandOptionValue(interaction, "user") || "").trim();
    const rawLimit = Number(getCommandOptionValue(interaction, "limit") || 5);
    const limit = Math.max(1, Math.min(10, rawLimit || 5));

    const attempts = requestedUserId
      ? await listVerificationAttemptsForUser(env, requestedUserId, limit)
      : await listRecentVerificationAttempts(env, limit);

    return createEphemeralMessageResponse(
      buildVerificationHistoryMessage(config, attempts, requestedUserId),
    );
  }

  if (commandName === "event_status") {
    if (!config.enableMonthlyEvents) {
      return createEphemeralMessageResponse("Eventy miesieczne sa aktualnie wylaczone.");
    }
    ...
    const limit = Math.max(1, Math.min(10, rawLimit || 5));
    const now = new Date();
    const dayKey = getLocalDayKey(now, config.resetTimezone);
    const monthKey = getLocalMonthKey(now, config.resetTimezone);
    const scheduledCount = await getMonthlyEventCount(env, monthKey);
    const upcomingEvents = await listUpcomingMonthlyEvents(env, dayKey, limit);

    return createEphemeralMessageResponse(
      buildEventStatusMessage(config, monthKey, dayKey, scheduledCount, upcomingEvents),
    );
  }
...
async function handleComponentInteraction(interaction, env, config) {
  ...
  if (customId === VERIFICATION_START_CUSTOM_ID) {
    ...
    try {
      return await handleVerificationStart(env, config, interaction);
    } catch (error) {
      console.error("Blad otwierania startu weryfikacji:", error);
      return createEphemeralMessageResponse(
        `Nie udalo sie uruchomic weryfikacji: ${String(error?.message || error)}`.slice(0, 1900),
      );
    }
  }
  ...
  if (customId.startsWith("verification:answer:")) {
    ...
    try {
      return await handleOpenAnswers(env, interaction, sessionId);
    } catch (error) {
      console.error("Blad otwierania formularza odpowiedzi:", error);
      return createEphemeralMessageResponse(
        `Nie udalo sie otworzyc formularza: ${String(error?.message || error)}`.slice(0, 1900),
      );
    }
  }
