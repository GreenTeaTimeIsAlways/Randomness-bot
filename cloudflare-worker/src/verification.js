...
async function finalizeNotPassed(config, interaction) {
  const guildId = String(interaction.guild_id);
  const userId = getUserId(interaction);
  const notes = [];
...
function formatResultMessage(config, session, evaluation, averageScore, resultCategory, roleNote = "") {
  ...
  if (resultCategory === "timeout") {
    lines.push("", "W trybie refleksu mozna podejsc od razu do kolejnej proby, jesli masz jeszcze wolne podejscia.");
  }
...
export async function processAnswersModal(env, config, interaction, sessionId) {
  ...
  if (resultCategory === "passed") {
    ...
    try {
      roleNote = await finalizeNotPassed(config, interaction);
    } catch (error) {
      roleNote = `Nie udalo sie ustawic roli startowej. Szczegoly: ${String(error?.message || error)}`;
    }
  }
