...
const PREVIEW_TOKEN_FIELD = "_previewInteractionToken";

function attachPreviewToken(challenge, interactionToken) {
  return {
    ...challenge,
    [PREVIEW_TOKEN_FIELD]: String(interactionToken || ""),
  };
}

function getPreviewToken(session) {
  return String(session?.challenge?.[PREVIEW_TOKEN_FIELD] || "").trim();
}

function getPublicChallenge(challenge) {
  const { [PREVIEW_TOKEN_FIELD]: _previewInteractionToken, ...publicChallenge } = challenge || {};
  return publicChallenge;
}

async function removeAnswerButtonFromPreview(config, session) {
  const previewToken = getPreviewToken(session);
  if (!previewToken) {
    return;
  }

  try {
    await editOriginalInteractionResponse(config, previewToken, {
      components: [],
    });
  } catch (error) {
    console.error("Nie udalo sie usunac starego przycisku odpowiedzi:", error);
  }
}
...
export async function processInterestModal(env, config, interaction) {
  ...
  try {
    ...
    challenge = attachPreviewToken(challenge, interactionToken);
...
export async function processAnswersModal(env, config, interaction, sessionId) {
  ...
  await removeAnswerButtonFromPreview(config, session);
  ...
  try {
    ...
    if (!evaluation) {
      if (session.challenge.mode === "bluff") {
        ...
        try {
          const rawEvaluation = await gradeOpenChallengeWithGemini(config, getPublicChallenge(session.challenge), answers);
  ...
  await recordAttempt(env, {
    ...
    createdAt: nowIso(),
    details: {
      challenge: getPublicChallenge(session.challenge),
      answers,
      evaluation,
    },
  });
