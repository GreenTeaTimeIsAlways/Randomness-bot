...
export async function listVerificationAttemptsForUser(env, userId, limit = 10) {
  const safeLimit = Math.max(1, Math.min(20, Number(limit) || 10));
  const result = await env.DB.prepare(
    `
      SELECT id, user_id, session_id, mode, interest, average_score, result, details_json, created_at
      FROM verification_attempts
      WHERE user_id = ?
      ORDER BY id DESC
      LIMIT ?
...
export async function listRecentVerificationAttempts(env, limit = 10) {
  const safeLimit = Math.max(1, Math.min(20, Number(limit) || 10));
  const result = await env.DB.prepare(
    `
      SELECT id, user_id, session_id, mode, interest, average_score, result, details_json, created_at
...
export async function listUpcomingMonthlyEvents(env, eventDate, limit = 5) {
  const safeLimit = Math.max(1, Math.min(10, Number(limit) || 5));
  const result = await env.DB.prepare(
    `
      SELECT id, month_key, day_number, event_date, event_kind, title, body, cta, payload_json, posted_at, created_at
      FROM monthly_events
      WHERE event_date >= ?
      ORDER BY event_date ASC, day_number ASC
      LIMIT ?
    `,
  )
    .bind(eventDate, safeLimit)
    .all();
